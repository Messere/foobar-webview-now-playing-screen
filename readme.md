...
be displayed on big TV screen while foobar is playing. To avoid screen damage
...
* It works well with local playback as well as streaming and playback from one
foobar2000 instance to anoter.
* Nice animation of spinning CD when audio is playing.
* Simple autogenerated "artwork" placeholder when no artwork is available (note:
this won't work if you have default artwork set in foobar2000).
* Custom CD artwork is supported, if no CD artwork is availabe then substitute is
...
interactive element allowing to toggle between play / pause on mouse click.
...
A section of configurable parameters is included on top of the template file in
`:root` css element:
